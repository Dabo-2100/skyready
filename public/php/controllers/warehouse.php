...
$endpoints += [
    ...
    '/api/warehouses/items/[\w-]+'      => 'items_index',
    '/api/warehouses/items/store'       => 'items_store',
    '/api/warehouses/cats'              => 'cats_index',
    '/api/warehouses/cats/store'        => 'cats_store',
    '/api/warehouses/units'             => 'units_index',
    '/api/warehouses/units/store'       => 'units_store',
    '/api/warehouses/packlists'         => 'packlists_index',
    '/api/warehouses/packlists/store'   => 'packlists_store',
];
...
{
    $page_id = explode("/api/warehouses/items/", $id[0])[1];
    ...
    if ($method === "GET") {
        ...
        try {
            if ($page_id == "all") {
                $items = getRows("warehouse_items", "1=1 ");
            } else {
...
function cats_index()
{
    ...
    if ($method === "GET") {
        $operator_info = checkAuth();
        try {
            $items = getRows("item_categories", "1=1");
            $response['data'] = $items;
            $response['err'] = false;
        } catch (\Throwable $th) {
            $response['msg'] = $th;
...
function cats_store()
{
    ...
    if ($method === "POST") {
        $operator_info = checkAuth();
        try {
            $warehouse_id = insert_data("item_categories", ["category_name"], [$POST_data['category_name']]);
            $response['err'] = false;
            $response['msg'] = 'New Category added successfully !';
            echo json_encode($response, true);
        } catch (\Throwable $th) {
            $response['msg'] = $th;
            echo json_encode($response, true);
...
function units_index()
{
    ...
    if ($method === "GET") {
        $operator_info = checkAuth();
        try {
            $items = getRows("warehouse_units", "1=1");
            $data = array_map(function ($el) {
                return $el;
            }, $items);
            $response['data'] = $data;
...
function units_store()
{
    global $method, $response, $POST_data;
    if ($method === "POST") {
        $operator_info = checkAuth();
        try {
            $unit_id = insert_data("warehouse_units", ["unit_name"], [$POST_data['unit_name']]);
            $response['err'] = false;
            $response['msg'] = 'New Unit added successfully !';
            echo json_encode($response, true);
        } catch (\Throwable $th) {
            $response['msg'] = $th;
            echo json_encode($response, true);
...
function packlists_index()
{
    global $method, $response;
    if ($method === "GET") {
        $operator_info = checkAuth();
        try {
            $items = getRows("warehouse_packlists", "1=1");
            $response['data'] = $items;
            $response['err'] = false;
        } catch (\Throwable $th) {
            $response['msg'] = $th;
...
function packlists_store()
{
    global $method, $response, $POST_data;
    if ($method === "POST") {
        $operator_info = checkAuth();
        try {
            $packlist_id = insert_data("warehouse_packlists", ["packlist_name"], [$POST_data['packlist_name']]);
            $response['err'] = false;
            $response['msg'] = 'New packlist added successfully !';
            echo json_encode($response, true);
        } catch (\Throwable $th) {
            $response['msg'] = $th;
            echo json_encode($response, true);
