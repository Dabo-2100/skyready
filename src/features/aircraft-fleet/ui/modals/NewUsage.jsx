...
export default function NewUsage() {
    const { addNewAircraftUsage, removeAircraftUsage } = useAircraft();
    const usages = useSelector(state => state.aircraftFleet.aircraftUsages.value);
    const new_usage_name = useRef();
    const handleSubmit = (event) => {
        event.preventDefault();
        addNewAircraftUsage(new_usage_name);
    }
                            ...
                            usages.map((el, index) => {
                                return (
                                    <tr key={el.usage_id}>
                                        ...
                                        <td>{el.usage_name}</td>
                                        <td>
                                            <FontAwesomeIcon icon={faX} onClick={() => removeAircraftUsage(el.usage_id)} className="removeManu btn btn-danger" />
                                        </td>
                                    </tr>
                                )
                            })
