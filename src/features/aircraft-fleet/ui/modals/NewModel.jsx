...
export default function NewModel() {
    const { addNewAircraftModel, removeAircraftModel } = useAircraft();
    const { aircraftModels: models } = useAircraftData();
    const new_model_name = useRef();

    const handleSubmit = (event) => {
        event.preventDefault();
        addNewAircraftModel(new_model_name);
    }
                            ...
                            models.map((el, index) => {
                                return (
                                    <tr key={el.model_id}>
                                        ...
                                        <td>{el.model_name}</td>
                                        <td>
                                            <FontAwesomeIcon icon={faX} onClick={() => removeAircraftModel(el.model_id)} className="removeManu btn btn-danger" />
                                        </td>
                                    </tr>
                                )
                            })
