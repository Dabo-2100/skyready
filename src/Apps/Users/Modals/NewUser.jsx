...
import { getAppRoles } from "../controller";
...
export default function NewUser() {
    ...
    const [selectedRoles, useSelectedRoles] = useState([]);
    ...
    const toggleRole = (app_id, role_id) => {
        ...
        if (roleIndex == -1) {
            useSelectedRoles([...selectedRoles, obj]);
        }
        else {
            if (role_id == -1) {
    ...
    const handleSubmit = () => {
        let formErrors = formCheck([
            { value: emailInput.current.value, options: { required: true } },
        ...
        if (formErrors == 0) {
            ...
            axios.post(`${serverUrl}/php/index.php/api/users/register`, data, { headers: { Authorization: `Bearer ${token}` } }).then((res) => {
                if (res.data.err) {
                    Swal.fire({
                        icon: "error",
                ...
                <main className="col-12 d-flex flex-wrap justify-content-center gap-2">
                    <div className="col-12 d-flex flex-wrap flex-md-nowrap justify-content-between gap-3 gap-md-0">
                        <div className="col-12 col-md-5 d-flex flex-wrap gap-1">
                            ...
                            <input ref={emailInput} type="email" className="form-control" />
                        </div>
                        <div className="col-12 col-md-5 d-flex flex-wrap gap-1">
                    ...
                    <div className="col-12 d-flex flex-wrap flex-md-nowrap align-itmes-end justify-content-between gap-3 gap-md-0">
                        <div className="col-12 col-md-5 d-flex flex-wrap gap-1">
                            ...
                            <select ref={specialty_id} className="form-select" defaultValue={-1}>
                        ...
                        <div className="col-12 col-md-5 d-flex gap-2 align-items-center">
                            ...
                            <input ref={is_super} type="checkbox" />
                        </div>
                                    ...
                                    apps.map((el, index) => {
                                        return (
                                            <tr key={el.app_id}>
                                                ...
                                                <td>{el.app_name}</td>
                                                <td>
                                                    <select className="form-select" defaultValue={-1} onChange={(event) => { toggleRole(el.app_id, event.target.value) }}>
