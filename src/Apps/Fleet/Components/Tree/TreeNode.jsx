...
const TreeNode = ({ node }) => {
    ...
    const { setParent_id, setOpenPackage_id } = useContext(FleetContext);

    const addNew = (event) => {
        event.stopPropagation();
        setParent_id(node.package_id);
        openModal(4000);
    }
        ...
        <div style={{ marginLeft: '0' }}>
            <div
...

                    {
                        node['is_folder'] == 0 ? (
                            <span className="link" onClick={() => {
                                setOpenPackage_id(node.package_id);
                                openModal(4002);
                ...
                <div className="d-flex align-items-center gap-3">
                    ...
                    {
                        node['is_folder'] == 1 && (
                            <button title="Controls" className="settingsButton" onClick={(event) => { addNew(event) }}>
