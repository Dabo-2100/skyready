...
import { useAircraftStatus } from "@/customHooks";
...
export default function NewStatus() {
    ...
    useEffect(() => { useAircraftStatus(serverUrl, token).then((res) => { setAircraftStatus(res) }) }, [])

    const handleSubmit = () => {
        event.preventDefault();
        let obj = {
            status_name: new_status_name.current.value,
        }
        new_status_name.current.value = "";
        axios.post(`${serverUrl}/php/index.php/api/aircraftstatus/store`, obj,
            {
                headers: {
            ...
            if (!res.data.err) {
                Swal.fire({
                    icon: "success",
                    text: "New Status Added Success !",
                    customClass: darkSwal,
                    timer: 1500,
                    showConfirmButton: false,
                }).then(() => {
                    setAircraftStatus(res.data.data);
                })
            }
            else {
                Swal.fire({
    ...
    const handleDelete = (stauts_id) => {
        Swal.fire({
            ...
            if (res.isConfirmed) {
                ...
                axios.post(`${serverUrl}/php/index.php/api/aircraftstatus/delete`, obj,
                    {
                        headers: {
                            Authorization: `Bearer ${token}`,
                        },
                                ...
                                aircraftStatus.map((el, index) => {
                                    return (
                                        <tr key={el.status_id}>
                                            ...
                                            <td>
                                                <FontAwesomeIcon icon={faX} onClick={() => handleDelete(el.status_id)} className="removeManu btn btn-danger" />
                                            </td>
                                        </tr>
                                    )
                                })
