...
import { useTaskTypes } from "@/customHooks";
...
export default function NewPackageTaskType() {
    ...
    const { specialty_id } = useContext(FleetContext);
    const [taskTypes, setTaskTypes] = useState([]);
    const new_type_name = useRef();
    useEffect(() => {
        useTaskTypes(serverUrl, token, specialty_id).then((res) => {
            setTaskTypes(res)
        })
    }, [])
    ...
    const handleSubmit = () => {
        ...
        let obj = {
            ...
            type_name: new_type_name.current.value
        }
        new_type_name.current.value = "";
        axios.post(`${serverUrl}/php/index.php/api/workpackage/tasks/types/store`, obj,
            {
                headers: {
    ...
    const handleDelete = (type_id) => {
        Swal.fire({
            ...
            if (res.isConfirmed) {
                ...
                axios.post(`${serverUrl}/php/index.php/api/workpackage/tasks/types/delete`, obj,
                    {
                        headers: {
                            Authorization: `Bearer ${token}`,
                        },
