...
import { useAircraftModels } from "@/customHooks";
...
export default function NewModel() {
    ...
    const new_model_name = useRef();

    useEffect(() => { useAircraftModels(serverUrl, token).then((res) => { setModels(res) }) }, [])

    const handleSubmit = () => {
        event.preventDefault();
        let obj = {
            model_name: new_model_name.current.value,
        }
        new_model_name.current.value = "";
        axios.post(`${serverUrl}/php/index.php/api/aircraftmodels/store`, obj,
            {
                headers: {
    ...
    const handleDelete = (model_id) => {
        Swal.fire({
            ...
            if (res.isConfirmed) {
                ...
                new_model_name.current.value = "";
                axios.post(`${serverUrl}/php/index.php/api/aircraftmodels/delete`, obj,
                    {
                        headers: {
                            Authorization: `Bearer ${token}`,
                        },
        ...
        <div className="modal">
            <div className="content animate__animated animate__fadeIn" onClick={(event) => { event.stopPropagation() }}>
                ...
                <header className="col-12 p-0 pb-2 px-3 d-flex align-content-center justify-content-between">
                    <h1 className="fs-5">Add New Model</h1>
                </header>
                <main className="col-12 d-flex flex-wrap">
                    <form className="col-12 d-flex flex-wrap gap-3" onSubmit={handleSubmit}>
                        <div className="col-12 inputField">
                            <label className="col-12 col-lg-5" htmlFor="mn">Model Name</label>
                            <div className="col-12 d-flex gap-4">
                                <input className="form-control" style={{ width: "70%" }} type="text" id="mn" ref={new_model_name} placeholder="Enter Model name" />
                ...
                <footer className="col-12 d-flex gap-3 flex-wrap align-items-center align-content-center p-3">
                    <p className="col-12">Aircraft Models list</p>
                                ...
                                models.map((el, index) => {
                                    return (
                                        <tr key={el.model_id}>
                                            ...
                                            <td>{el.model_name}</td>
                                            <td>
                                                <FontAwesomeIcon icon={faX} onClick={() => handleDelete(el.model_id)} className="removeManu btn btn-danger" />
                                            </td>
                                        </tr>
                                    )
                                })
