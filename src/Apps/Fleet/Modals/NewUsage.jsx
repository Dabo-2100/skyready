...
import { useAircraftUsages } from "@/customHooks";
...
export default function NewUsage() {
    ...
    const new_usage_name = useRef();
    useEffect(() => { useAircraftUsages(serverUrl, token).then((res) => { setUsages(res) }) }, [])

    const handleSubmit = () => {
        event.preventDefault();
        let obj = {
            usage_name: new_usage_name.current.value,
        }
        new_usage_name.current.value = "";
        axios.post(`${serverUrl}/php/index.php/api/aircraftusages/store`, obj,
            {
                headers: {
    ...
    const handleDelete = (usage_id) => {
        Swal.fire({
            icon: "question",
            text: "Are you sure you want to delete this Usage ?",
            ...
            if (res.isConfirmed) {
                ...
                new_usage_name.current.value = "";
                axios.post(`${serverUrl}/php/index.php/api/aircraftusages/delete`, obj,
                    {
                        headers: {
                            Authorization: `Bearer ${token}`,
                        },
        ...
        <div className="modal">
            <div className="content animate__animated animate__fadeIn" onClick={(event) => { event.stopPropagation() }}>
                ...
                <header className="col-12 p-0 pb-2 px-3 d-flex align-content-center justify-content-between">
                    <h1 className="fs-5">Add New Usage</h1>
                </header>
                <main className="col-12 d-flex flex-wrap">
                    <form className="col-12 d-flex flex-wrap gap-3" onSubmit={handleSubmit}>
                        <div className="col-12 inputField">
                            <label className="col-12 col-lg-5" htmlFor="mn">Usage Name</label>
                            <div className="col-12 d-flex gap-4">
                                <input className="form-control" style={{ width: "70%" }} type="text" id="mn" ref={new_usage_name} placeholder="Enter Usage name" />
                ...
                <footer className="col-12 d-flex gap-3 flex-wrap align-items-center align-content-center p-3">
                    <p className="col-12">Aircraft Usages list</p>
                                ...
                                usages.map((el, index) => {
                                    return (
                                        <tr key={el.usage_id}>
                                            ...
                                            <td>{el.usage_name}</td>
                                            <td>
                                                <FontAwesomeIcon icon={faX} onClick={() => handleDelete(el.usage_id)} className="removeManu btn btn-danger" />
                                            </td>
                                        </tr>
                                    )
                                })
